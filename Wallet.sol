...
contract Wallet {
    ...
    mapping(address => mapping (address => uint256)) allowed_eth; // Хеш таблица где счет А дает доступ счету Б доступ к нек. кол-ву токенов
    ...
    function changeFee(uint newFee) public {
        require(totalEthAmount <= totalEthAmount/10);
        eth_fee = newFee;
    }
